...
def draw_character_dialogues(character_dialogues, season: int):
    ...
    plt.pie(dialogues, labels=characters, autopct='%1.1f%%', startangle=140, colors=plt.cm.Paired.colors, textprops={'fontsize': 20})
...
def date_analysis(df : pd.DataFrame):
    ...
    plt.bar(years, episode_count, color='pink')
    ...
    plt.figure(figsize=(10, 6))
    plt.bar(years, view_count, color='pink')
...
def analyze_viewers_per_writer(df: pd.DataFrame):
    ...
    plt.ylabel('Total US Viewers (in millions)', fontsize=18)
...
def seasons_viewing_analysis(df: pd.DataFrame): 
    ...
    plt.xticks(season_views['season'], fontsize=18)
    plt.yticks(fontsize=18)
    plt.grid()
    plt.tight_layout()
...
def writer_character_correlation(df: pd.DataFrame):
    ...
    plt.ylabel('Writers', fontsize=18)
    plt.title('Correlation between the appearance of a character and a writer', fontsize=18)
    plt.tight_layout()
...
def episode_ranking(df: pd.DataFrame, top_n=20): 
    ...
    plt.xlabel('Total US Viewers (in millions)', fontsize=18)
